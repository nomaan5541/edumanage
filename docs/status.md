...
Tauri desktop shell — PARTIAL (merged to main via #6; CSP follow-up on this branch) — impl: official Tauri 2 window around the existing Vite/React app (`src-tauri/` loads `../dist`; `npm run tauri:dev` / `tauri:build`); 1280×800 window with min 1024×640; placeholder icons; Rust is window-only (no Supabase, RLS, or permission checks); CSP is a non-null policy (self/asset scripts, IPC, HTTPS/WSS + localhost for env-driven Supabase, `object-src 'none'`) — db: n/a — RLS: n/a (same server-side rules as web) — roles: all (same routes as web, including `/login`) — tests: `npm run test:desktop` is static config only (does not boot the desktop app or reach Supabase); `npm run build` and `npm run lint` for the web target — issues: not CURRENT. No hosted Supabase project / `.env`, so live login-to-API was not proven. Bundles are unsigned. A Linux `tauri build` before the CSP change produced unsigned deb/rpm/AppImage; the window titled EduManage opened with a blank webview, so the login form was not proven. This environment has no `rustc`/`cargo`; the post-CSP binary was not compiled or run here. Do not treat scaffolding as a signed installer or as a proven login UI.
