...
Audit Logs — PARTIAL — impl: write_audit_log() RPC, called from create_school_bootstrap; AuditLogPage viewer UI — db: audit_logs table, append-only grants — RLS: select-only policy written — roles: super_admin, school_admin — routes: /admin/audit-log, /super-admin/audit-log — tests: none run yet — issues: school/school_admin creation, fee catalog RPCs, and subscription activate/extend/cancel write audit events; other modules' RPCs still need to call write_audit_log per docs/conventions/BACKEND_CONVENTIONS.md.
...
## Financial
Fees/Fee Structures/Payments — PARTIAL — impl: school-scoped `fee_types` + `fee_structures` catalog (class + academic year, no student FK) with create/update RPCs, audit, and School Admin UI — db: `20260917120000_fee_types_and_structures.sql` — RLS: SELECT for school members; INSERT/UPDATE/DELETE revoked (RPC-only writes) — roles: school_admin / sub_admin with `fees.create` or `fees.update` — routes: /admin/fees — UI: local Vite at /admin/fees (create Tuition type, structures tab) — tests: `supabase/tests/fee_catalog_and_subscriptions.sql` passed after `npx supabase db reset` on local Docker Postgres (catalog SEC-001/002 isolation, teacher deny, ungated sub-admin deny, granted sub-admin create, cross-school structure trigger, exact read-only write rejection while SELECT still works). PAY-001..007 were not run: no `students` table, no dues, no `fee_payments`, no `record_fee_payment`. — issues: student-linked dues/payments/receipts deferred until a `students` table exists.
Receipts (PDF) — REQUIRED — deferred until student-linked payments exist.
Subscription (manual Super Admin activation) — PARTIAL — impl: Super-Admin-only `activate_school_subscription` / `extend_school_subscription` / `cancel_school_subscription` on existing `public.subscriptions.plan_name` (no `subscription_plans` table; Phase 1 records plan as text) plus UI at /super-admin/subscriptions — db: `20260917120100_subscription_admin_rpcs.sql` — RLS: still no client write policy on `subscriptions` — roles: super_admin only; school_admin denied — routes: /super-admin/subscriptions — UI: local Vite at /super-admin/subscriptions (school card + Activate dialog) — tests: same SQL file (school_admin RPC + table write denied, activate/extend/cancel, audit events `subscription.activated|extended|cancelled`, extend after cancel restores writes). SUBS-002/003 (view students / export while expired) not applicable yet. — issues: no priced plan catalog, no subscription_requests, no in-app billing (skipped by design).
